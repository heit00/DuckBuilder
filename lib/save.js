...
class Query {
    ...
    #executePendingStates(){
        const pendingStates = this.#pendingStates;
        pendingStates.forEach((el) => {
            if(typeof el !== 'function') throw new Error('internal Error: conflict on pendingState.');
            else el();
        })
    }
    ...
    #set() {
        ...
        if (action === undefined) Error('action not defined.');
    ...
    get selectColumns() {
        if (this.#action !== QueryGrammar.actions.select) return 0;
    ...
    insert(table) {
        if (!this.#action || this.#action === QueryGrammar.actions.insert) this.#action = QueryGrammar.actions.insert;
        else throw new QuerySyntaxError(`a action is already registered: (${this.#action})`);
        //clear()
    ...
    set(left, right) {
        if (this.#action === QueryGrammar.actions.update)
            this.#updateSets.set(left, right);
        else if (this.#action === QueryGrammar.actions.insert)
    ...
    toInstruction(count, mode) {
        ...
        if (!mode) {
            ...
            if (this.#action !== QueryGrammar.actions.select && this.#returningColumns.length === 0) throw new QuerySyntaxError('in CTE you need to return a data set.');
    ...
    #toInstruction(count) {
        ...
        else if (this.#action === QueryGrammar.actions.update) {
