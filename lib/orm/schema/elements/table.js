...
const { ST, ELEMENT_VALUE_TYPE, isColumn, isTable } = require('../../global-symbol-lockup/symbols');
...
class TableSchema {
  ...
  #updateMetaData(column) {
    if (column.primary()) {
      ...
      if(this.constraints.has(constraint.name())) throw new Error(`constraint ${constraint.name()} already registered.`);
    ...
    if (column.unique()) {
      const constraint = new Constraint().type(Constraint.TYPES.unique).columns(column.name());
      constraint.unique(column.name()).name(`${this.schemaName}_${this.name}_${column.name()}`, Constraint.PREFIX.unique);
      if(this.constraints.has(constraint.name())) throw new Error(`constraint ${constraint.name()} already registered.`);
      this.constraints.set(constraint.name(), constraint);
    }
    if (!column.nullable()) {
  ...
  manyToOneRelation(target, references = {}, name, { constraintName, ...options } = {}) {
    ...
    const cName = constraintName || `${Relationship.PREFIX.foreign}_${relName}`;
  ...
  manyToManyRelation(name, target, originReferences, targetReferences, pivot = null, pivotName) {
    if (!isTable(target)) throw new TypeError('target must be Table schema instance.');
    if (!originReferences || typeof originReferences !== 'object' || !targetReferences || typeof targetReferences !== 'object') {
      throw new TypeError('all references args must be objects.');
    }

    if (!pivot) {
      const finalPivotName = pivotName ? pivotName : `${this.schemaName}.${this.name}_relationIntermediate_${target.schemaName}.${target.name}`;
      pivot = new TableSchema(finalPivotName);
      createIntermediateTableColumns(originReferences, pivot, this);
      createIntermediateTableColumns(targetReferences, pivot, target);
    }

    createIntermediateTableReferences(originReferences, pivot, this);
    ...
    this.appendRelation(relationship);
...
function populateColumnsArray(columns, references, target) {
...
function createIntermediateTableColumns(references, pivot, target) {
  ...
  if (!Array.isArray(references)) {
    populateColumnsArray(columns, references, target);
  } else {
    references.forEach(ref => {
      populateColumnsArray(columns, ref, target);
    });
  }
