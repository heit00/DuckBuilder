...
class OrderBy {
    ...
    constructor(column, type = QueryGrammar.orderByTypes.asc) {
        ...
        else if (!(isColumn(column)) && !(isCase(column)) && !(isQuery(column)) && !(isRaw(column))) throw new TypeError('column must be a string, a instance of Column class or a caseClause instance.');
