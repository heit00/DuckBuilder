const { isValue, isColumn, isRaw, isSelect, IT, IR } = require('../symbol-lockup/symbols');
...
class SchemaStructure {
    ...
    get valuesMetaSet() {
        ...
        if (this.#type === IT.select.instance) {
            if (!isSelect(this.#data[0])) throw new QuerySyntaxError('Can not has non-select statement in INSERT-SELECT statement.');
    ...
    select(query, ...columns) {
        ...
        if (!isSelect(query) && !isRaw(query)) throw new TypeError('you can only use raw and query objects.');
