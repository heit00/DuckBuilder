...
const {Expression, InExpression} = require('../lexicalStructures/booleanable');
...
const {WhereRaw} = require('../lexicalStructures/raw');
...
class WhereClause {
    ...
    #add(left, operator, right, type) {
        ...
        if (right === undefined) {
            if(operator === undefined) throw new TypeError('can not has operator undefined.');
            right = operator;
            operator = '=';
        }
    ...
    whereRaw(raw, ...values){
        return this.#addRaw(QueryGrammar.logicalOperator.and, raw, ...values);
    }

    orWhereRaw(raw, ...values){
        return this.#addRaw(QueryGrammar.logicalOperator.or, raw, ...values);
    }

    #addRaw(type, raw, ...values){
        const whereRaw = new WhereRaw(type, raw, values);
        this.#expressions.push(whereRaw);
        return this;
    }
