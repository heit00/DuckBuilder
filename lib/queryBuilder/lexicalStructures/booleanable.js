...
class InBooleanable {
    ...
    constructor(left, right, type = QueryGrammar.logicalOperator.and, isNot = false) {
        ...
        else if (isQuery(right) || isRaw(right)) {
            this.#right = right;
        }
...
class Booleanable {
    ...
    constructor(left, operator, right, type = QueryGrammar.logicalOperator.and) {
    ...
    toInstruction(hiddenType = false, count) {
        if (typeof hiddenType !== 'boolean') throw new TypeError('hiddenType must be a boolean.');
        let preString;
        const leftInst = this.#left.toInstruction(count, IR.cte);
        const rightInst = this.#right.toInstruction(count, IR.cte);
...
module.exports = { Expression: Booleanable, InExpression: InBooleanable };